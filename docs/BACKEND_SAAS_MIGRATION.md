...
## Frontend status (admin-suman)

| Tələb | Status |
|-------|--------|
| Login: `license_code` sahəsi | ✅ `app/login/page.tsx` |
| Token + user (`company_id`, `company_name`) | ✅ `AuthContext`, Sidebar |
...
- [x] Login formuna **`license_code`** sahəsi əlavə edin (mətn, böyük hərf göstərmək olar).
- [x] `token` + `user` saxlayın; sonrakı bütün sorğularda `Authorization: Bearer <token>`.
- [x] `user.company_id` / `user.company_name` istəsəniz UI-da göstərin.
- [x] **`POST /api/auth/register` bağlıdır** — 403. Qeydiyyat səhifəsi lazım deyilsə silin.
...
- [x] Login: 3 sahə (email, şifrə, lisenziya kodu)
- [x] Sifariş cədvəlində nişə + `!is_paid` → «Ödənilməyib» badge + **«Borc ödə»** düyməsi → `mark-paid`
- [x] Tarixçə kartlarında `unpaidCreditAmount` ayrıca göstərilsin
- [x] 409 telefon xətası istifadəçiyə aydın mesajla
...
Əlavə frontend qeydləri: `docs/BACKEND_ADMIN_TASKS.md`
