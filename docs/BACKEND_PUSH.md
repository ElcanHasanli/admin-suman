...
(Köhnə: `POST /api/notifications/device-token` — yalnız kuryer, `device_tokens`-ə də yazır.)
...
3. Kuryer sifariş tamamlayır → hər iki cihazda bildiriş

```sql
SELECT user_id, platform, app, left(token, 20) FROM device_tokens WHERE app = 'admin';
```

Admin tətbiq tərəfi: `docs/PUSH-ADMIN.md`
